...
//    std::vector<SCIP_CONS*> newCons(nCons_);
    ...
    auto* transprobdatavrp = new ProbDataVRP(modelData_, nVars_, nCons_);
...
)
{
    ...
    auto* prob = new ProbDataVRP(modeldata, 0, modeldata->nC - 1 + modeldata->nDays);
...
//        cons[i] = con;
    ...
    /* add constraint, such that at most one route is choosen for every day */
    ...
    }else /* else use initial heuristic */
    {

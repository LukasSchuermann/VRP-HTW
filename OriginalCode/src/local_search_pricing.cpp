...
bool addNodeToTourPricing(
        ...
        tourVRP&                            tvrp,
        const vector<vector<vector<bool>>>& isForbidden,
        int                                 cust,
        int                                 *newpos,
        SCIP_Real                           threshold
){
    ...
    if(SCIPisSumNegative(scip, extracosts - bestcosts))
    {
        ...
        if(!isForbidden[day][0][cust] && !isForbidden[day][cust][tmpTour.tour_[1]])
        {
    ...
    for(i = 1; i < tvrp.length_ + 1; i++)
    {
        ...
        /* check for branching decisions */
        if(isForbidden[day][tmpTour.tour_[i-1]][cust] || isForbidden[day][cust][tmpTour.tour_[i+1]])
            continue;
...
//        cust = p.first;
        ...
        for(int pos = 0; pos < tvrp.length_; pos++)
        {
            ...
            if(pos == 0)
            {
                if(pricerData->isForbidden_[day][0][cust])
                    continue;
                if(tvrp.length_ > 1 && pricerData->isForbidden_[day][cust][tvrp.tour_[pos+1]])
                    continue;
            }
            else if(pos == tvrp.length_ - 1)
            {
                if(pricerData->isForbidden_[day][tvrp.tour_[pos-1]][cust] ||
                   pricerData->isForbidden_[day][cust][0])
                    continue;
            }else
            {
                if(pricerData->isForbidden_[day][tvrp.tour_[pos-1]][cust] ||
                   pricerData->isForbidden_[day][cust][tvrp.tour_[pos+1]])
                    continue;
            }
            /* costs of exchange */
...
bool decreaseColumn(
    ...
    for(i = 0; i < tvrp.length_; i++)
    {
        ...
        if(i == tvrp.length_ - 1)
        {
            if(pricerData->isForbidden_[day][prev][0])
            {
                prev = cust;
                continue;
            }
        }else{
            if(pricerData->isForbidden_[day][prev][tvrp.tour_[i+1]])
            {
                prev = cust;
                continue;
            }

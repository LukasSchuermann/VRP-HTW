...
SCIP_RETCODE getToursFromLabelPairs(
    ...
    {
        if(count > 20 && !SCIPinProbing(scip))
            break;
    ...
    while(labelLists[current_index]->length_ > 0)
    {
        ...
        if(calcRedCosts || curr_label2->time_ <= (double) modelData->timeWindows[0][day].end / 2) // TODO: can we make it sharp? - Prob. no!
        {
            ...
            for(auto neighbor : neighbors)
            {
                ...
                if(noDominance && !pricerData->dc_nodes_[neighbor])
                {
                    dom_active = 0;
                    dom_old = 0;
                }else
    ...
    for(auto i : modelData->neighbors[0][day])
    {
        ...
        while (fw_node != nullptr)
        {
            for(auto j : modelData->neighbors[0][day])
            {
                    ...
                    if(SCIPisSumNegative(scip, rc))
                    {
                        if(isFeasible(modelData, day, fw_node->label2_, bw_node->label2_))
                        {
                            if(SCIPisSumNegative(scip, rc))
                            {
                                ...
                                count++;
                                if(count >= 25 && !SCIPinProbing(scip)) //TODO: nAdded Vars
                                    break;
                            }
                        }
                    }
                ...
                if(count >= 25 && !SCIPinProbing(scip))
                    break;
            }
...
)
{
    ...
    if(SCIPinProbing(scip)){
        assert(pricerData->cons_cpc_ != nullptr);
        if(pricerData->cons_cpc_->withCuts_){
            pricerData->enumerate_ = true;
        }
    }
...
//        thread_args[i].bestTours = vector<tourVRP>();
    ...
    if(pricerData->cons_cpc_ != nullptr){
        if(SCIPinProbing(scip))
            pricerData->cons_cpc_->nAdded_Vars_ = 0;
    }
...
//                            cout << "FOUND FIX TOUR (" << SCIPnodeGetNumber(SCIPgetFocusNode(scip)) << ")" << endl;
                ...
                if(pricerData->cons_cpc_ != nullptr)
                    pricerData->cons_cpc_->nAdded_Vars_++;
            }
            ...
            count++;
            // During Probing we want to collect all variables
            if(!SCIPinProbing(scip) && count > probData->getData()->nC / 4)
            {
                break;
            }
        }

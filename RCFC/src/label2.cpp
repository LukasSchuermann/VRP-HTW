...
//            return nullptr;
    ...
    else if(pricerData->enumerate_){
        /* if in enumerate mode, we need to generate elementary routes --> NG(L) = V(L) */
        ng_memory = old_label->ng_memory_;
    }else{
...
//        red_costs += modelData->travel[start][end];
    ...
    new_label = new Label(red_costs, end, capacity, pricerData->nnonzSRC_, new_time, obj, ng_memory, old_label->visitedEC_);
    if(isEnforced)

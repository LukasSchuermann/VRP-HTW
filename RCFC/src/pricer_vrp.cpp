...
//        {
//            pricerData->isForbidden_[day][i][j] = FALSE;
//        }
    }
...
SCIP_DECL_PRICERINIT(ObjPricerVRP::scip_init)
{
    ...
    if(SCIPfindConshdlr(scip, "RCFC") != nullptr){
        cons_cpc_ = dynamic_cast<ConshdlrRCFC*>(SCIPgetObjConshdlr(scip, SCIPfindConshdlr(scip, "RCFC")));
    }
    node_varsfixed_ = -1;
    ...
    dc_nodes_.resize(modelData->nC, true);
    ...
    )
{
    ...
    if(cons_cpc_ != nullptr){
        if(SCIPinProbing(scip))
        {
            assert(cons_cpc_ != nullptr);
            auto end_t = std::chrono::high_resolution_clock::now();
            cons_cpc_->lp_solving_time_ += std::chrono::duration_cast<std::chrono::microseconds>(end_t - cons_cpc_->start_time_).count();
            if(SCIPisLT(scip, SCIPgetLPObjval(scip), cons_cpc_->obj_cmp_))
            {
                return SCIP_OKAY;
            }
            /* set customers for which dominance check must not be performed */
            for(int i = 0; i < probData->getData()->nC; i++){
                dc_nodes_[i] = true;
            }
            for(auto v : prop_tourfixing_->tvrp_.tour_){
                dc_nodes_[v] = false;
            }


//            std::cout << "Start pricing iteration " << std::endl;
            cons_cpc_->used_Pricing_ = true;
            start_time_ = std::chrono::high_resolution_clock::now();
        }else
        {
            if(cons_cpc_->nonViolatedCut_)
            {
                cons_cpc_->nonViolatedCut_ = false;
                return SCIP_OKAY;
            }
        }
    }
    ...
    if(lastID_ != currNode && !SCIPinProbing(scip))
    {
        ...
        bool sepfirst = false;
        if(tree_data_[parent_ID].branchtype_ >= 3)
        {
            sepfirst = true;
        }
        ...
        tree_data_[currNode].node_lowest_fail = tree_data_[parent_ID].node_lowest_fail; //TODO: which factor to use?
...
//    if(!SCIPinProbing(scip))
//        std::cout << "pricing at node " << currNode << " with LP: " << SCIPgetLPObjval(scip) << std::endl;
...
//    if(!SCIPinProbing(scip))
//    {
//        std::cout << "Exact pricing at node " << currNode << " gap " << 100  * (SCIPgetCutoffbound(scip) - SCIPgetLPObjval(scip)) / SCIPgetLPObjval(scip) << std::endl;
//    }
    /* Exact pricing */
    auto start = std::chrono::high_resolution_clock::now();
    nvars = probData->nVars_;
    SCIP_CALL( generate_tours(scip, probData, isFarkas, calcRedCosts, false));
...
//    if(SCIPinProbing(scip))
//    {
//        std::cout << "added " << probData->nVars_ - nvars << " variables in pricing round" << std::endl;
//    }
    /* save ng_dssr_data for finished branching node */
    if(SCIPinProbing(scip))
    {
        auto end_time = std::chrono::high_resolution_clock::now();
//        std::cout << "time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time_).count();
        cons_cpc_->pricing_time_ += std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time_).count();
    }
    if(nvars == probData->nVars_)
    {
//        if(probData->depth_gaps_.size() < SCIPgetDepth(scip) + 1){
//            probData->depth_gaps_.emplace_back();
//        }
//        double loc_gap = std::max((SCIPgetCutoffbound(scip) - SCIPgetLPObjval(scip)) / SCIPgetLPObjval(scip), 0.0);
//        probData->depth_gaps_[SCIPgetDepth(scip)].push_back(loc_gap);
        if(SCIPgetDepth(scip) == 0 && cons_cpc_ != nullptr){
            cons_cpc_->root_gap_ =  100  * (SCIPgetCutoffbound(scip) - SCIPgetLPObjval(scip)) / SCIPgetLPObjval(scip);
        }
    ...
    if(!PARALLEL_LABELING)
    {
        ...
        for(int i = 0; i < probData->getData()->nDays; i++)
        {
            ...
            if(SCIPinProbing(scip) && i == prop_tourfixing_->tvrp_.getDay() && prop_tourfixing_->up_)
                SCIP_CALL(generateLabelsBiDir(scip, probData->getData(), this, bestTours, isFarkas, getDayVarRed, false, true, i, false));
            else
                SCIP_CALL(generateLabelsBiDir(scip, probData->getData(), this, bestTours, isFarkas, getDayVarRed, false, false, i, false));

//            if(SCIPinProbing(scip))
//            {
//                auto end_time = std::chrono::high_resolution_clock::now();
////        std::cout << "time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time_).count();
//                std::cout << "TIME: " << std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time_).count();
//                vector<tourVRP> bestTours2;
//                start_time_ = std::chrono::high_resolution_clock::now();
//                enumerate_ = true;
//                SCIP_CALL(generateLabelsBiDir(scip, probData->getData(), this, bestTours2, isFarkas, getDayVarRed, false, true, i, false));
//                end_time = std::chrono::high_resolution_clock::now();
//                std::cout << " ---> " << std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time_).count() << std::endl;
//                enumerate_= false;
//            }
            ...
            for(auto& tvrp : bestTours)
            {
                ...
                if(SCIPinProbing(scip))
                    cons_cpc_->nAdded_Vars_++;

                count++;
                if(!SCIPinProbing(scip) && count > probData->getData()->nC / 4) // TODO: what is a good criteria?
                {
                    break;
                }
            }

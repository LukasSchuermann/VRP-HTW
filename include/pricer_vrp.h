...
enum BranchType
{
    ...
    BRANCHTYPE_VEHICLEARC = 3,
    BRANCHTYPE_VEHICLE = 4,
    BRANCHTYPE_NVEHICLE = 5
};
...
typedef struct node_data {
    ...
    vector<vector<vector<bool>>>        node_isForbidden_; /**< isForbidden for branching nodes */
    double                              node_lowest_fail;
} node_data;
...
public:
    ...
    SCIP_CONSHDLR*                          cons_veharc_;    /**< constraint handler for day branching */
    ...
    vector<vector<vector<bool>>>            isForbidden_;    /**< matrix that indicates if an arc between two customers if forbidden due to arc flow branching */
    vector<vector<vector<bool>>>            global_isForbidden_; /**< matrix that indicates if an arc between two customers if forbidden due to arc flow branching */
    ...
    vector<vector<vector<SCIP_Real>>>       arcRedCosts_;    /**< reduced costs of arc flow variables */
    vector<vector<vector<SCIP_Real>>>       root_arcRedCosts_;    /**< reduced costs of arc flow variables */
    vector<vector<vector<SCIP_Real>>>       root_arcLPObj_;    /**< reduced costs of arc flow variables */
    ...
    bool                                    atRoot_;
    bool                                    enumerate_;     /**< is enumerate mode activated */
    /** parameters for reduced cost fixing */
    double                                  gap_decay_rc_;     /**< percentage decrease of the gap since last call */
    int                                     max_depth_rc_;     /**< max depth to call fixing */
    int                                     maxSRC_rc_;        /**< max number of SRCs to call with */
    long                                    labelRCTime_ = 0;
